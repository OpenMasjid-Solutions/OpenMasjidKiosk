...
import { CalendarDays, Coins, Download, MonitorSmartphone, ReceiptText, TrendingUp } from 'lucide-react';
      ...
      <section className="glass panel">
        ...
        {donations.length === 0 ? (
          ...
          <ul className="donation-list">
            {donations.map((d) => (
              <DonationRow key={d.id} d={d} money={money} />
            ))}
          </ul>
        )}
      </section>
...
function DonationRow({ d, money }: { d: Donation; money: (m: number) => string }) {
  const succeeded = d.status === 'succeeded';
  const who = d.donorName || d.donorEmail;
  return (
    <li className="donation-row">
      ...
      <div className="donation-row__meta muted">
        {who ? `${who} · ` : ''}{d.deviceName || 'Kiosk'} · {relativeTime(d.createdAt)}
      </div>
    </li>
  );
}
