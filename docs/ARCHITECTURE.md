...
| **WhatsApp pacing** | `whatsappGate` in `alerts.ts`, `raiseAlert` in `index.ts` | OpenMasjidOS 0.51.1 removed every send limit it used to impose (per-recipient cooldown, caps, quiet hours, the warm-up ramp). That silently deleted a backstop this app depended on without knowing: `payment-failed` fires per refused PaymentIntent and has no natural bound, so a Stripe outage during jummah is one message per attempted donation. Ban risk attaches to the NUMBER, is shared by every app on the box, and is unrecoverable — so the pacing is ours now: one per alert id per 30 min, with the suppressed count carried on the next message. Refusals are stored per alert and shown in the admin panel; the 202's `id` is resolved to sent/failed via the 0.51.1 status endpoint. |
